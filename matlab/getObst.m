...
switch idx
	...
	case 12 % Three circles of radius 1/2
        dist = 2; % Previously 4 but too far
		par = struct('obsts', [struct('par', @(t) [sin(2*pi*t); cos(2*pi*t)]/2, 'gradnorm',... 
			@(t) pi*ones(size(t)), 'normal', @(t) [sin(2*pi*t); cos(2*pi*t)], 'corners', [], 'dbf', 1, 'ppw', 10, ...
            'serpar', @(t,mo) [(2*pi).^(0:mo-1)./factorial(0:mo-1).*sin(2*pi*t + (0:mo-1)*pi/2); ... 
            (2*pi).^(0:mo-1)./factorial(0:mo-1).*cos(2*pi*t + (0:mo-1)*pi/2)]/2) ... %End obst 1
			struct('par', @(t) repmat([0; dist], 1, length(t)) + [sin(2*pi*t); -cos(2*pi*t)]/2, 'gradnorm',...
			@(t) pi*ones(size(t)), 'normal', @(t) [sin(2*pi*t); -cos(2*pi*t)], 'corners', [], 'dbf', 1, 'ppw', 10, ...
            'serpar', @(t,mo) [(2*pi).^(0:mo-1)./factorial(0:mo-1).*sin(2*pi*t + (0:mo-1)*pi/2); ...
            (2*dist*((0:mo-1) == 0) -(2*pi).^(0:mo-1)./factorial(0:mo-1).*cos(2*pi*t + (0:mo-1)*pi/2))]/2), ... % End obst 2
            struct('par', @(t) repmat([dist/2*tan(pi/3); dist/2], 1, length(t)) + [sin(2*pi*t); -cos(2*pi*t)]/2, 'gradnorm',...
			@(t) pi*ones(size(t)), 'normal', @(t) [sin(2*pi*t); -cos(2*pi*t)], 'corners', [], 'dbf', 1, 'ppw', 10, ...
            'serpar', @(t,mo) [(dist*tan(pi/3)*((0:mo-1) == 0)+(2*pi).^(0:mo-1)./factorial(0:mo-1).*sin(2*pi*t + (0:mo-1)*pi/2)); ...
            (dist*((0:mo-1) == 0)-(2*pi).^(0:mo-1)./factorial(0:mo-1).*cos(2*pi*t + (0:mo-1)*pi/2))]/2)], 'bc', @(k,x) -1*exp(1i*k*(x')*[cos(0); sin(0)]), ...
            'xi', 3e-3);
		return
